...
ZEND_VM_COLD_CONST_HANDLER(203, ZEND_NOT_NULL_ASSERTION, ANY, UNUSED)
{
	USE_OPLINE
	SAVE_OPLINE();

	zval *value = GET_OP1_OBJ_ZVAL_PTR_DEREF(BP_VAR_R);
	if (EG(exception)) {
		HANDLE_EXCEPTION();
	}
	if (Z_TYPE_P(value) == IS_NULL) {
		zend_throw_error(NULL, "Encountered unexpected null value");
		FREE_OP1();
		HANDLE_EXCEPTION();
	}
