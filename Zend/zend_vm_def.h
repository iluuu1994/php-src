...
ZEND_VM_HELPER(zend_add_helper, ANY, ANY, zval *op_1, zval *op_2)
{
	...
	SAVE_OPLINE();
	if (UNEXPECTED(Z_ISUNDEF_P(op_1))) {
		op_1 = ZVAL_UNDEFINED_OP1();
	}
	if (UNEXPECTED(Z_ISUNDEF_P(op_2))) {
...
ZEND_VM_COLD_CONSTCONST_HANDLER(3, ZEND_MUL, CONST|TMPVARCV, CONST|TMPVARCV, SPEC(COMMUTATIVE))
{
	...
	if (ZEND_VM_SPEC && OP1_TYPE == IS_CONST && OP2_TYPE == IS_CONST) {
		...
		if (EXPECTED(Z_TYPE_P(op2) == IS_LONG)) {
			...
			result = EX_VAR(opline->result.var);
			ZEND_SIGNED_MULTIPLY_LONG(Z_LVAL_P(op1), Z_LVAL_P(op2), l, d1, overflow);
			if (EXPECTED(!overflow)) {
				ZVAL_LONG(result, l);
			} else {
				ZVAL_DOUBLE(result, d1);
			}
			ZEND_VM_NEXT_OPCODE();
		} else if (EXPECTED(Z_TYPE_P(op2) == IS_DOUBLE)) {
			d1 = (double)Z_LVAL_P(op1);
...
ZEND_VM_C_LABEL(assign_dim_op_new_array):
		...
		if (EXPECTED(Z_TYPE_P(container) == IS_OBJECT)) {
			...
			if (OP1_TYPE == IS_CV && UNEXPECTED(Z_ISUNDEF_P(container))) {
				ZVAL_UNDEFINED_OP1();
			}
			ht = zend_new_array(8);
...
ZEND_VM_HOT_NOCONST_HANDLER(43, ZEND_JMPZ, CONST|TMP|CV, JMP_ADDR)
{
	...
	if (Z_TYPE_P(val) == IS_TRUE) {
		...
		if (OP1_TYPE == IS_CV && UNEXPECTED(Z_ISUNDEF_P(val))) {
			SAVE_OPLINE();
			ZVAL_UNDEFINED_OP1();
...
ZEND_VM_HANDLER(109, ZEND_FETCH_CLASS, UNUSED|CLASS_FETCH, CONST|TMP|UNUSED|CV, CACHE_SLOT)
{
	...
	SAVE_OPLINE();
	if (OP2_TYPE == IS_UNUSED) {
		ZVAL_CE(EX_VAR(opline->result.var), zend_fetch_class(NULL, opline->op1.num));
		ZEND_VM_NEXT_OPCODE_CHECK_EXCEPTION();
		...
		ZVAL_CE(EX_VAR(opline->result.var), ce);
	} else {
		class_name = GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R);
ZEND_VM_C_LABEL(try_class_name):
		if (Z_TYPE_P(class_name) == IS_OBJECT) {
			ZVAL_CE(EX_VAR(opline->result.var), Z_OBJCE_P(class_name));
		} else if (Z_TYPE_P(class_name) == IS_STRING) {
			ZVAL_CE(EX_VAR(opline->result.var), zend_fetch_class(Z_STR_P(class_name), opline->op1.num));
...
ZEND_VM_HANDLER(139, ZEND_GENERATOR_CREATE, ANY, ANY)
{
	...
	if (EXPECTED(return_value)) {
		...
		ZEND_CALL_INFO(gen_execute_data) = call_info;
...
ZEND_VM_HOT_HANDLER(117, ZEND_SEND_VAR, VAR|CV, CONST|UNUSED|NUM)
{
	...
	if (OP1_TYPE == IS_CV && UNEXPECTED(Z_ISUNDEF_P(varptr))) {
		SAVE_OPLINE();
		ZVAL_UNDEFINED_OP1();
		ZVAL_NULL(arg);
		ZEND_VM_NEXT_OPCODE_CHECK_EXCEPTION();
...
ZEND_VM_HANDLER(73, ZEND_INCLUDE_OR_EVAL, CONST|TMP|CV, ANY, EVAL, SPEC(OBSERVER))
{
	...
	if (UNEXPECTED(EG(exception) != NULL)) {
		...
		call = zend_vm_stack_push_call_frame(
			(hasThis() ? ZEND_CALL_HAS_THIS : 0) | ZEND_CALL_NESTED_CODE | ZEND_CALL_HAS_SYMBOL_TABLE,
...
ZEND_VM_HELPER(zend_fe_fetch_object_helper, ANY, ANY)
{
	...
	if ((iter = zend_iterator_unwrap(array)) == NULL) {
		...
		if (RETURN_VALUE_USED(opline)) {
			if (UNEXPECTED(Z_TYPE(p->key) == IS_LONG)) {
				ZVAL_LONG(EX_VAR(opline->result.var), Z_LVAL(p->key));
			} else if (Z_STRVAL(p->key)[0]) {
				ZVAL_STR_COPY(EX_VAR(opline->result.var), Z_STR(p->key));
			} else {
...
ZEND_VM_C_LABEL(fe_fetch_r_exit):
	...
	if (EXPECTED(OP2_TYPE == IS_CV)) {
		...
		ZVAL_DEREF(value);
		ZVAL_COPY(EX_VAR(opline->op2.var), value);
	}
	ZEND_VM_NEXT_OPCODE_CHECK_EXCEPTION();
}
								...
								if (prop_info) {
									if (UNEXPECTED(prop_info->flags & ZEND_ACC_READONLY)) {
										zend_throw_error(NULL,
											...
											ZSTR_VAL(prop_info->ce->name), Z_STRVAL(p->key));
										UNDEF_RESULT();
										HANDLE_EXCEPTION();
									}
									if (ZEND_TYPE_IS_SET(prop_info->type)) {
...
ZEND_VM_HOT_TYPE_SPEC_HANDLER(ZEND_PRE_INC, (res_info == MAY_BE_LONG && op1_info == MAY_BE_LONG), ZEND_PRE_INC_LONG_NO_OVERFLOW, CV, ANY, SPEC(RETVAL))
{
	...
	var_ptr = GET_OP1_ZVAL_PTR_PTR_UNDEF(BP_VAR_RW);
	ZVAL_LONG(var_ptr, Z_LVAL_P(var_ptr) + 1);
	if (UNEXPECTED(RETURN_VALUE_USED(opline))) {
		ZVAL_LONG(EX_VAR(opline->result.var), Z_LVAL_P(var_ptr));
	}
	ZEND_VM_NEXT_OPCODE();
