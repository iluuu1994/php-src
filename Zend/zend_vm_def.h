...
ZEND_VM_HANDLER(29, ZEND_ASSIGN_STATIC_PROP_OP, ANY, ANY, OP)
{
	...
	prop = zend_fetch_static_property_address(&prop_info, (opline+1)->extended_value, BP_VAR_RW, 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
	if (UNEXPECTED(!prop)) {
		UNDEF_RESULT();
		FREE_OP_DATA();
		HANDLE_EXCEPTION();
...
ZEND_VM_HELPER(zend_pre_incdec_obj_helper, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, bool is_inc)
{
	USE_OPLINE
	zval *object;
	zval *property;
...
ZEND_VM_C_LABEL(pre_incdec_object):
		...
		if (EXPECTED((zptr = zobj->handlers->get_property_ptr_ptr(zobj, name, BP_VAR_RW, cache_slot)) != NULL)) {
			if (UNEXPECTED(Z_ISERROR_P(zptr))) {
				...
				prop_info = (zend_property_info *) CACHED_PTR_EX(cache_slot + 2);
				zend_pre_incdec_property_zval(zptr, prop_info, is_inc, opline->result.var OPLINE_CC EXECUTE_DATA_CC);
			}
		} else {
...
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_pre_incdec_obj_helper, is_inc, true);
}

ZEND_VM_HANDLER(133, ZEND_PRE_DEC_OBJ, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, CACHE_SLOT)
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_pre_incdec_obj_helper, is_inc, false);
}

ZEND_VM_HELPER(zend_post_incdec_obj_helper, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, bool is_inc)
{
	USE_OPLINE
	zval *object;
	zval *property;
...
ZEND_VM_C_LABEL(post_incdec_object):
		...
		if (EXPECTED((zptr = zobj->handlers->get_property_ptr_ptr(zobj, name, BP_VAR_RW, cache_slot)) != NULL)) {
			if (UNEXPECTED(Z_ISERROR_P(zptr))) {
				...
				prop_info = (zend_property_info*)CACHED_PTR_EX(cache_slot + 2);
				zend_post_incdec_property_zval(zptr, prop_info, is_inc, opline->result.var OPLINE_CC EXECUTE_DATA_CC);
			}
		} else {
...
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_post_incdec_obj_helper, is_inc, true);
}

ZEND_VM_HANDLER(135, ZEND_POST_DEC_OBJ, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, CACHE_SLOT)
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_post_incdec_obj_helper, is_inc, false);
}

/* No specialization for op_types (CONST|TMPVAR|CV, UNUSED|CONST|VAR) */
ZEND_VM_HELPER(zend_pre_incdec_static_prop_helper, ANY, ANY, OPTIONS(quick_op_flags_field=op_data), bool is_inc)
{
	USE_OPLINE
	zval *prop;
	zend_property_info *prop_info;
	...
	prop = zend_fetch_static_property_address(&prop_info, opline->extended_value, BP_VAR_RW, 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
	if (UNEXPECTED(!prop)) {
		UNDEF_RESULT();
		HANDLE_EXCEPTION();
	}
	...
	zend_pre_incdec_property_zval(prop,
		ZEND_TYPE_IS_SET(prop_info->type) ? prop_info : NULL, is_inc, opline->result.var OPLINE_CC EXECUTE_DATA_CC);
...
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_pre_incdec_static_prop_helper, is_inc, true);
}

/* No specialization for op_types (CONST|TMPVAR|CV, UNUSED|CONST|VAR) */
ZEND_VM_HANDLER(39, ZEND_PRE_DEC_STATIC_PROP, ANY, ANY, CACHE_SLOT)
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_pre_incdec_static_prop_helper, is_inc, false);
}

/* No specialization for op_types (CONST|TMPVAR|CV, UNUSED|CONST|VAR) */
ZEND_VM_HELPER(zend_post_incdec_static_prop_helper, ANY, ANY, OPTIONS(quick_op_flags_field=op_data), bool is_inc)
{
	USE_OPLINE
	zval *prop;
	zend_property_info *prop_info;
...
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_post_incdec_static_prop_helper, is_inc, true);
}

/* No specialization for op_types (CONST|TMPVAR|CV, UNUSED|CONST|VAR) */
ZEND_VM_HANDLER(41, ZEND_POST_DEC_STATIC_PROP, ANY, ANY, CACHE_SLOT)
{
	ZEND_VM_DISPATCH_TO_HELPER(zend_post_incdec_static_prop_helper, is_inc, false);
}
...
/* No specialization for op_types (CONST|TMPVAR|CV, UNUSED|CONST|VAR) */
ZEND_VM_INLINE_HELPER(zend_fetch_static_prop_helper, ANY, ANY, OPTIONS(quick_op_flags_field=op_data), int type)
{
	USE_OPLINE
	zval *prop;
	zend_property_info *prop_info;
	...
	prop = zend_fetch_static_property_address(
		...
		type == BP_VAR_W ? opline->extended_value : 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
	if (UNEXPECTED(!prop)) {
...
ZEND_VM_HANDLER(25, ZEND_ASSIGN_STATIC_PROP, ANY, ANY, CACHE_SLOT, SPEC(OP_DATA=CONST|TMP|VAR|CV))
{
	...
	prop = zend_fetch_static_property_address(&prop_info, opline->extended_value, BP_VAR_W, 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
	if (UNEXPECTED(!prop)) {
		FREE_OP_DATA();
		UNDEF_RESULT();
		HANDLE_EXCEPTION();
...
ZEND_VM_HANDLER(33, ZEND_ASSIGN_STATIC_PROP_REF, ANY, ANY, CACHE_SLOT|SRC)
{
	...
	prop = zend_fetch_static_property_address(&prop_info, opline->extended_value & ~ZEND_RETURNS_FUNCTION, BP_VAR_W, 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
	if (UNEXPECTED(!prop)) {
		FREE_OP_DATA();
		UNDEF_RESULT();
		HANDLE_EXCEPTION();
...
ZEND_VM_HANDLER(68, ZEND_NEW, UNUSED|CLASS_FETCH|CONST|VAR, UNUSED|CACHE_SLOT, NUM)
{
	...
	if (constructor == NULL) {
		 ...
		 * opcode is DO_FCALL in case EXT instructions are used. We store this info in op1
		 * of slim_op for DO_FCALL and DO_FCALL_BY_NAME. */
		if (EXPECTED(opline->extended_value == 0 && (opline+1)->op1.num == ZEND_DO_FCALL)) {
			ZEND_VM_NEXT_OPCODE_EX(1, 2);
		}
...
ZEND_VM_HELPER(zend_fe_fetch_object_helper, ANY, ANY, OPTIONS(quick_op_flags_field=op_data))
{
	USE_OPLINE
	zval *array;
	zval *value;
...
ZEND_VM_HANDLER(180, ZEND_ISSET_ISEMPTY_STATIC_PROP, ANY, CLASS_FETCH, ISSET|CACHE_SLOT)
{
	...
	value = zend_fetch_static_property_address(NULL, opline->extended_value & ~ZEND_ISEMPTY, BP_VAR_IS, 0, OP1_TYPE, opline->op1.var, OP2_TYPE, opline->op2.var OPLINE_CC EXECUTE_DATA_CC);
