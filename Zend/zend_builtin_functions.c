...
ZEND_API void zend_fetch_debug_backtrace(zval *return_value, int skip_last, int options, int limit) /* {{{ */
{
	...
	while (call && (limit == 0 || frameno < limit)) {
		...
		if (ZEND_USER_CODE(call->func->type)) {
			const zend_op *opline = EX_OPLINE_EX(call);
			...
			if (call->func && ZEND_USER_CODE(call->func->common.type)) {
				...
				zend_op *call_op = EX_OPLINE_EX(call);
				if (call_op->opcode == ZEND_HANDLE_EXCEPTION) {
					if (EG(opline_before_exception)) {
						lineno = OP_S2W_EX(&call->func->op_array, EG(opline_before_exception))->lineno;
					} else {
...
not_frameless_call:
		...
		if (prev && prev->func && ZEND_USER_CODE(prev->func->common.type)) {
			...
			zend_op *prev_op = EX_OPLINE_EX(prev);
			if (prev_op->opcode == ZEND_HANDLE_EXCEPTION) {
				if (EG(opline_before_exception)) {
					lineno = OP_S2W_EX(&prev->func->op_array, EG(opline_before_exception))->lineno;
				} else {
			...
			while (prev_call) {
				...
				if (prev && prev->func && ZEND_USER_CODE(prev->func->common.type)) {
					...
					_zend_hash_append_ex(stack_frame, ZSTR_KNOWN(ZEND_STR_FILE), &tmp, 1);
					ZVAL_LONG(&tmp, EX_OPLINE_EX(prev)->lineno);
					_zend_hash_append_ex(stack_frame, ZSTR_KNOWN(ZEND_STR_LINE), &tmp, 1);
					break;
				}
				prev_call = prev;
		...
		if (!fake_frame && func->common.function_name) {
			...
			if (prev && prev->func && ZEND_USER_CODE(prev->func->common.type) && EX_OPLINE_EX(prev)->opcode == ZEND_INCLUDE_OR_EVAL) {
...
skip_frame:
		if (UNEXPECTED(ZEND_CALL_KIND(call) == ZEND_CALL_TOP_FUNCTION)
		 ...
		 && EX_OPLINE_EX(prev)->opcode == ZEND_INCLUDE_OR_EVAL) {
			fake_frame = 1;
		} else {
			fake_frame = 0;
