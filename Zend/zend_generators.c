...
try_again:
	...
	/* Resume execution */
#if ZEND_DEBUG
	zend_op *op = EX_OPLINE_EX(generator->execute_data);
	...
	if (UNEXPECTED((generator != orig_generator && !Z_ISUNDEF(generator->retval)) || (generator->execute_data && EX_OPLINE_EX(generator->execute_data)->opcode == ZEND_YIELD_FROM))) {
		generator = zend_generator_get_current(orig_generator);
		goto try_again;
	}
