...
php_sha512_crypt_r(const char *key, const char *salt, char *buffer, int buflen) {
	...
	if (((uint64_t) key) % __alignof__ (uint64_t) != 0) {
		char *tmp = (char *) alloca (key_len + __alignof__ (uint64_t));
		key = copied_key =
		memcpy(tmp + __alignof__(uint64_t) - ((uint64_t) tmp) % __alignof__(uint64_t), key, key_len);
	}

	if (((uint64_t) salt) % __alignof__ (uint64_t) != 0) {
		char *tmp = (char *) alloca(salt_len + 1 + __alignof__(uint64_t));
		salt = copied_salt = memcpy(tmp + __alignof__(uint64_t) - ((uint64_t) tmp) % __alignof__(uint64_t), salt, salt_len);
