...
static uint32_t count_unterminated_init_fn(zend_op_array *op_array, zend_basic_block *b)
{
	...
	return num_do_call - num_init_fn;
}

static void make_unterminated_init_fn_nop(zend_cfg *cfg, zend_op_array *op_array, zend_basic_block *removed_b)
{
	zend_basic_block *blocks = cfg->blocks;
	uint32_t num_init_fn_to_delete = count_unterminated_init_fn(op_array, removed_b);
	if (num_init_fn_to_delete <= 0) {
		return;
	}

	zend_bool nesting = 0;
	uint32_t num_fn_init_deleted = 0;
	for (zend_basic_block *b = removed_b - 1; b >= blocks; b--) {
		zend_op *op = op_array->opcodes + b->start + b->len;
		zend_op *start = op - b->len;
		for (; op >= start; op--) {
			zend_bool is_init_fn = op->opcode == ZEND_INIT_FCALL
				|| op->opcode == ZEND_INIT_FCALL_BY_NAME
				|| op->opcode == ZEND_INIT_NS_FCALL_BY_NAME
				|| op->opcode == ZEND_INIT_DYNAMIC_CALL
				|| op->opcode == ZEND_INIT_USER_CALL
			...
			if (nesting == 0 && is_init_fn) {
				MAKE_NOP(op);
				num_fn_init_deleted++;

				if (num_fn_init_deleted >= num_init_fn_to_delete) {
					return;
				}
...
static void assemble_code_blocks(zend_cfg *cfg, zend_op_array *op_array, zend_optimizer_ctx *ctx)
{
	...
	for (b = blocks; b < end; b++) {
		...
		if (b->flags & ZEND_BB_REACHABLE) {
			...
			make_unterminated_init_fn_nop(cfg, op_array, b);
		}
	}
